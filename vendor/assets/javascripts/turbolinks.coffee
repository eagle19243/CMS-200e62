# CUSTOMIZED BY JACK JENNINGS - 10 FEB 2015
# Inserts a transition between new page fetch and load

# Custom Config

transitionSpeed       = 3300
pageSlideOutModifier  = 2.8
patternFadeDuration   = 750
footerDropoutDuration = 500
...
startTransition = ->
  return new $.Deferred().resolve() if isMobile()
  ...
  slideOutSpeed      = $window.height() * pageSlideOutModifier
  transitionDelay    = slideOutSpeed / 3
  distanceToCover    = $animation.height() + $window.height()
  endTransitionDelay = transitionSpeed - transitionDelay * 4

  runAnimation $animation, transitionSpeed, transitionDelay

  $('body').addClass 'hide-overflow'
           .css paddingRight: measure_scrollbar()

  $pattern.animate opacity: 0, patternFadeDuration

  $footer.animate marginTop: 500, footerDropoutDuration
         .promise().done -> $(this).css opacity: 0

  $content.transitionOut slideOutSpeed
          .delay endTransitionDelay
          .promise()
...
endTransition = ->
  ...
  $content.transitionIn slideOutSpeed
          .promise()
          .done ->
...
isTransitioning = ->
  $animation = $('.transition-animation')
  $animation.is(":animated") || $('.wrapper').is(":animated")

# Below is slightly modified Tubrolinks that inserts animation promises
# between page loads...
...
fetchReplacement = (url, onLoadFunction, showProgressBar = true) ->
  ...
  xhr.onloadend = -> animation.done -> xhr = null
...
cacheCurrentPage = ->
  ...
  pageCache[currentStateUrl.absolute] =
    ...
    body:                     $(document.body).clone().get(0),
...
changePage = (title, body, csrfToken, runScripts) ->
  ...
  $body = $(body)
  replacement = $body.find('.content-to-replace')

  unless isMobile()
    replacement.css opacity: 0
    replacement.find('.pattern-animation')
               .css opacity: 0
               .animate opacity: 1

  document.title = title
  $('.content-to-replace').replaceWith replacement
  $('body').removeClass().addClass $body.attr('class')
  ...
  endTransition().done -> triggerEvent EVENTS.COMPLETE
...
class Click
  ...
  constructor: (@event) ->
    ...
    return @event.preventDefault() if $('body').is(':animated')
    @_extractLink()
